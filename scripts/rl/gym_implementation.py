...
class TradingEnvGym(gym.Env):
    ...
    def step(self, action):
        ...
        if isinstance(action, (list, tuple, np.ndarray)):
            ...
            # In case someone passes a scalar, map to [dir, size]
...
def train(cfg: TrainConfig):
    ...
    env = TradingEnvGym(returns, EnvConfig(window=cfg.window, max_steps=cfg.max_steps))

    state_dim = cfg.window
    branch_sizes = list(env.action_space.nvec.tolist())
        ...
        while not done:
            ...
            if len(rb) >= cfg.warmup and (global_step % cfg.steps_per_update == 0):
                ...
                if global_step % 1000 == 0:
                    print(f"step {global_step} loss {loss.item():.6f} eps {eps:.3f} ep_reward {ep_reward:.4f}")
...
def main():
    p = argparse.ArgumentParser(description='Gymnasium BDQ (DeepScalper-style)')
